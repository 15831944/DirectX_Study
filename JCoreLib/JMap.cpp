...
bool JMap::Create(ID3D11Device* device, int iWidth, int iHeight, int iCellSize, const TCHAR* szShaderName, const char* szVSFuntionName, const char* szPSFuntionName )
{
	m_iNumCols = iWidth;
	m_iNumRows = iHeight;
	...
	m_dxObj.m_pVS = DX::LoadVertexShader(g_pd3dDevice, szShaderName, &m_dxObj.m_BlobVS, szVSFuntionName);
	m_dxObj.m_pPS = DX::LoadPixelShader(g_pd3dDevice,  szShaderName, &m_dxObj.m_BlobPS, szPSFuntionName);
...
bool JMap::PreRender(ID3D11DeviceContext* context, UINT iVertexSize)
{
	context->UpdateSubresource(m_dxObj.m_pConstantBuffer, 0, NULL, &m_cbData, 0, 0);
	m_dxObj.PreRender(context, iVertexSize);
	return true;
}
bool JMap::Render(ID3D11DeviceContext* context, UINT iVertexSize, UINT iCount)
{
	PreRender(context, iVertexSize);
	PostRender(context, iCount);
	return true;
}
bool JMap::PostRender(ID3D11DeviceContext* context, UINT iCount)
{
