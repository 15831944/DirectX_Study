...
namespace DX
{
	...
	ID3D11VertexShader* LoadVertexShader(ID3D11Device* device, const TCHAR* szLoadFileName, ID3DBlob** ppBlobOut, const char* szShaderFunc)
	{
		...
		if (szShaderFunc == 0)
		{
			if (FAILED(hr = CompileShader(device, szLoadFileName, &pBlob, "VS", "vs_5_0")))
			{
				return nullptr;
			}
		}
		...
		{
			if (FAILED(hr = CompileShader(device, szLoadFileName, &pBlob, szShaderFunc, "vs_5_0")))
			{
				return nullptr;
			}
		}
	...
	ID3D11PixelShader* LoadPixelShader(ID3D11Device* device, const TCHAR* szLoadFileName, ID3DBlob** ppBlobOut, const char* szShaderFunc)
	{
		...
		if (szShaderFunc == 0)
		{
			if (FAILED(hr = CompileShader(device, szLoadFileName, &pBlob, "PS", "ps_5_0")))
			{
				return nullptr;
			}
		}
		...
		{
			if (FAILED(hr = CompileShader(device, szLoadFileName, &pBlob, szShaderFunc, "ps_5_0")))
			{
				return nullptr;
			}
		}
	...
	ID3D11InputLayout* CreateLayout(ID3D11Device* device, D3D11_INPUT_ELEMENT_DESC* IED, int ElementCnt, ID3DBlob* pVSBlob)
	{
		...
		DWORD dwSize = pVSBlob->GetBufferSize();
	...
	HRESULT CompileShader(ID3D11Device* device, const TCHAR* szLoadFileName, ID3DBlob** pBlob, const char* szShaderFunc, const char* szShaderModel)
	{
		HRESULT hr = S_OK;
