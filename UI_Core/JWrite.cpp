...
bool JWrite::GreenDraw(const TCHAR* text, float size, fRECT rt)
{
	CreateTextFormat(size);
	D2D1_RECT_F WriteRT;
	WriteRT.left = rt.left;
	...
	WriteRT.top = rt.top;
	Begin();
	m_pRT->DrawTextW(text, wcslen(text), m_pTextFormat, WriteRT, m_pGreenBrush);
	End();
	return true;
}
bool JWrite::SetColor()
{
	...
	m_pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::LawnGreen), &m_pGreenBrush);
	return true;
}
