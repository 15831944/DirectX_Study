...
HRESULT	DxRT::Create(ID3D11Device* device, FLOAT Width, FLOAT Height, ID3D11Texture2D* pTex)
{
	HRESULT hr = S_OK;
	...
	{
		if (FAILED(hr = device->CreateRenderTargetView(pTex, NULL, &m_pRTV)))
		{
			return hr;
		}
	}
...
HRESULT DxRT::CreateCubeMap(ID3D11Device*	device, FLOAT fWidth, FLOAT fHeight)
{
	return S_OK;
}
void DxRT::ViewPortSet(ID3D11Device* device, FLOAT TopLeftX, FLOAT TopLeftY, FLOAT Width, FLOAT Height)
{
...
HRESULT DxRT::UpdateDepthStencilView(ID3D11Device* pDevice, UINT iWidth, UINT iHeight)
{
	HRESULT hr;
	if (m_pDSV != NULL)
	{
...
void	DxRT::Apply(ID3D11DeviceContext* context, ID3D11RenderTargetView* pRTV, ID3D11DepthStencilView* pDSV, bool bDepth, bool bStencil)
{
...
bool	DxRT::Begin(ID3D11DeviceContext* context, D3DXVECTOR4 vColor, bool bTarget, bool bDepth, bool bStencil)
{
	m_iVp = 1;
...
bool	DxRT::Clear(ID3D11DeviceContext* context, D3DXVECTOR4 vColor, bool bTarget, bool bDepth, bool bStencil)
{
	if (bTarget)
	{
...
bool	DxRT::End(ID3D11DeviceContext* context)
{
