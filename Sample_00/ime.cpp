...
ime* ime::Instance = nullptr;
static wstring g_aszIndicator[] =
{
	TEXT("W"),
#ifdef UNICODE
	L"\x7B80",
...
BOOL ime::OnImeComposition(WPARAM wParam, LPARAM lParam)
{
	HIMC	hIMC;
...
int ime::WndProc(const MSG msg)
{
	...
	case WM_IME_ENDCOMPOSITION:
		m_CompWord = msg.wParam;
		return 0;
		// 편집중에 있는 스트링을 얻는다.
	case WM_IME_COMPOSITION:
	{
		OnImeComposition(msg.wParam, msg.lParam);
	}
	return 1;
	//case WM_IME_CHAR:
	//	m_CompString += wParam;
	...
	//case WM_IME_NOTIFY:
	//{
	//	if (msg.wParam == IMN_SETCONVERSIONMODE)
	//	{
	//		HIMC hIMC;
	//		DWORD dwConversion;
	//		DWORD dwSentence;
	...
	//					MessageBox(msg.hwnd, L"한글 완성형 모드", L"IME Status", MB_OK);
	//				else
	//					MessageBox(msg.hwnd, L"한글 조합형 모드", L"IME Status", MB_OK);
	//			}
	//			else									// 영문 mode
	//			{

	//				MessageBox(msg.hwnd, L"영문 모드", L"IME Status", MB_OK);
	//			}
...
void ime::Initialize(HWND hWnd)
{
	ImeUi_Initialize(hWnd);
}
//--------------------------------------------------------------------------------------
void ime::Uninitialize()
{
	ImeUi_Uninitialize();
}

bool ime::ImeUi_IsEnabled(void)
{
	return m_bInitialized;
}

bool ime::ImeUi_Initialize(HWND hwnd, bool bDisable)
{
	if (m_bInitialized)
	{
		return true;
...
void ime::SetIMECompPos(int xpos, int ypos)
{
	HIMC hImc = ImmGetContext(m_hWndMain);
	if (hImc) {
		COMPOSITIONFORM cf;
...
const TCHAR* ime::GetStringBuffer()
{
	m_CurrentString.clear();
	m_CurrentString = m_CompString + m_CompWord;
	return m_CurrentString.c_str();
}
ime::ime()
{
	m_bInitialized = false;
	m_hImmDll = NULL;
	m_pszIndicatior = g_aszIndicator[0];
	m_dwConversion = 0;//IME_CMODE_ALPHANUMERIC
}
