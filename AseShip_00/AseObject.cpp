...
{
	if (!LoadAseFile(AseFileName)) return false;
...
bool AseObject::LoadTexFile(ID3D11Device* device, string szTexName)
{
	TCHAR Ret[256] = { 0, };
...
bool AseObject::Create(ID3D11Device* device)
{
	for (int iMtrl = 0; iMtrl < m_Mtrl.size(); iMtrl++)
	{
		if (m_Mtrl[iMtrl].subMtrl.size() > 0)
		{
			for (int iSubMtrl = 0; iSubMtrl < m_Mtrl[iMtrl].subMtrl.size(); iSubMtrl++)
			{
				...
				m_SubAseObj[iSubMtrl].m_dxObj.m_iVertexSize = sizeof(PNCT_VERTEX);
				...
				//tex
				m_SubAseObj[iSubMtrl].LoadTexFile(device, m_Mtrl[iMtrl].subMtrl[iSubMtrl].tex[0].strTexName);
		...
		else
		{
			...
			//tex
			LoadTexFile(device, m_Mtrl[iMtrl].tex[0].strTexName);
...
vector<PNCT_VERTEX> AseObject::CutTriangle(vector<ASE_TRIANGLE> triangle)
{
	vector<PNCT_VERTEX> retTri;
...
void AseObject::SetMatrix(D3DXMATRIX* matWorld, D3DXMATRIX* matView, D3DXMATRIX* matProj)
{
	for (int iMtrl = 0; iMtrl < m_Mtrl.size(); iMtrl++)
	{
		if (m_Mtrl[iMtrl].subMtrl.size() > 0)
		{
			for (int iSubMtrl = 0; iSubMtrl < m_Mtrl[iMtrl].subMtrl.size(); iSubMtrl++)
			{
				m_SubAseObj[iSubMtrl].JShape::SetMatrix(matWorld, matView, matProj);
			}
		}
		else
		{
			JShape::SetMatrix(matWorld, matView, matProj);
		}
	}
}
AseObject::AseObject()
