...
VS_OUTPUT VS(PNCT_INPUT vIn)
{
	...
	Output.n = vIn.n;
	Output.c = vIn.c;
	Output.t.x = (vIn.p.x + 1.0f) / 2.0f;	// x값은 -1 ~ 1의 값을 0~1로 변환 
	Output.t.y = (-vIn.p.y + 1.0f) / 2.0f;	// y값은 1 ~ -1의 값을 0~1로 변환
	//Output.t.x = (vIn.p.x * 0.5f) + 0.5f;
	//Output.t.y = (-vIn.p.y * 0.5f) + 0.5f;
	return Output;
}
