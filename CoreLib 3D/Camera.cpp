...
#pragma endregion
	...
	{
		m_curPosition = Lerp(m_curPosition, m_pParent->GetWorldPosition(), spf);
		m_curRotation = Lerp(m_curRotation, m_pParent->GetWorldRotation(), spf);
...
void Camera::SetParent(GameObject* pParent)
{
	m_pParent = pParent;
	if (pParent == nullptr)
		return;
...
void Camera::SetOrthoWH(const float& width, const float& height)
{
	D3DXMatrixOrthoLH(&m_matProj, width, height, 0.0f, 200.0f);
	UpdateMatrix();
}

void Camera::isPerspect(const bool& isPerspect)
{
	m_isPerspect = isPerspect;
	SetProjMatrix();
	UpdateMatrix();
